...
type Registry interface {
	...
	CreateCustomResource(ctx genericapirequest.Context, customResource *unstructured.Unstructured) (*unstructured.Unstructured, error)
	UpdateCustomResource(ctx genericapirequest.Context, customResource *unstructured.Unstructured) (*unstructured.Unstructured, error)
...
func (s *storage) CreateCustomResource(ctx genericapirequest.Context, customResource *unstructured.Unstructured) (*unstructured.Unstructured, error) {
	obj, err := s.Create(ctx, customResource, false)
	if err != nil {
		return nil, err
	}
	return obj.(*unstructured.Unstructured), nil
...
func (s *storage) UpdateCustomResource(ctx genericapirequest.Context, customResource *unstructured.Unstructured) (*unstructured.Unstructured, error) {
	obj, _, err := s.Update(ctx, customResource.GetName(), rest.DefaultUpdatedObjectInfo(customResource, api.Scheme))
	if err != nil {
		return nil, err
	}
	return obj.(*unstructured.Unstructured), nil
