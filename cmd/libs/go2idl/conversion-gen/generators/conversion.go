...
func (e equalMemoryTypes) Equal(a, b *types.Type, existingTypes map[*types.Type]bool) bool {
	if a == b {
		return true
	}
	...
	result := e.equal(a, b, existingTypes)
...
func (e equalMemoryTypes) equal(a, b *types.Type, existingTypes map[*types.Type]bool) bool {
	...
	case in.Kind == out.Kind:
		// avoid recursion for recursive types
		if existingTypes[in] {
			return true
		}
		existingTypes[in] = true
		switch in.Kind {
		case types.Struct:
			...
			for i, inMember := range in.Members {
				...
				if !e.Equal(inMember.Type, outMember.Type, existingTypes) {
					return false
				}
			}
			return true
		case types.Pointer:
			return e.Equal(in.Elem, out.Elem, existingTypes)
		case types.Map:
			return e.Equal(in.Key, out.Key, existingTypes) && e.Equal(in.Elem, out.Elem, existingTypes)
		case types.Slice:
			return e.Equal(in.Elem, out.Elem, existingTypes)
...
type TypesEqual interface {
	Equal(a, b *types.Type, existingTypes map[*types.Type]bool) bool
}
...
func (g *genConversion) doStruct(inType, outType *types.Type, sw *generator.SnippetWriter) {
	for _, inMember := range inType.Members {
		...
		// existingTypes holds all the types that have been converted. This is used to avoid
		// recursion for recursive types.
		existingTypes := make(map[*types.Type]bool)
		if g.useUnsafe.Equal(inMemberType, outMemberType, existingTypes) {
			args = args.
