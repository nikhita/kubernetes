...
func setupQuotaController(t *testing.T, kubeClient kubernetes.Interface, lister quota.ListerForResourceFunc) quotaController {
	...
	tweakableRM.Add(schema.GroupVersionKind{Group: "example.com", Version: "v1", Kind: "foobars"}, nil)
		...
		quota := &v1.ResourceQuota{
			...
			Status: v1.ResourceQuotaStatus{
				...
				Used: v1.ResourceList{
					"count/foobars.example.com": resource.MustParse("3"),
				},
			},
		}

		qc.addQuota(quota)
		fmt.Println(qc.queue.Len())

		quota2 := quota
		quota2.Spec.Hard = v1.ResourceList{
			"count/foobars.example.com": resource.MustParse("2"),
		}
		fmt.Println("syncing")

		qc.syncResourceQuota(quota2)
		fmt.Println(qc.queue.Len())
...
func TestSyncResourceQuota(t *testing.T) {
	...
	for testName, testCase := range testCases {
		...
		usage, err := kubeClient.Core().ResourceQuotas("default").Get("rq", metav1.GetOptions{})
		if err != nil {
			t.Error(err)
		}
...
func TestAddQuota(t *testing.T) {
		...
		},
		{
			...
			quota: &v1.ResourceQuota{
				...
				Status: v1.ResourceQuotaStatus{
					...
					Used: v1.ResourceList{
						"foobars.example.com": resource.MustParse("3"),
					},
				},
			},
		},
	...
	for _, tc := range testCases {
		...
		fmt.Println("syncing againg...")
		qc.syncResourceQuota(tc.quota)
...
func TestQuotaForCustomResource(t *testing.T) {
	...
	customResource := map[schema.GroupVersionResource]struct{}{
		{Group: "example.com", Version: "v1", Resource: "foobars"}: {},
	}
	...
	resourceQuotaControllerOptions := &ResourceQuotaControllerOptions{
		...
		DynamicClient:             dynamicClient,
		Mapper:                    rm,
		QuotableResources:         customResource,
		SharedInformerFactory:     sharedInformers,
	}
	qc, err := NewResourceQuotaController(resourceQuotaControllerOptions)
	if err != nil {
	...
	_, err = qc.rqClient.ResourceQuotas("default").Create(fooBarQuota)
	if err != nil {
		t.Errorf("could not create quota")
	}

	if err := qc.syncResourceQuota(fooBarQuota); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	_, err = qc.rqClient.ResourceQuotas("default").Get("rq", metav1.GetOptions{})
	if err != nil {
		t.Errorf("could not get quota quota")
	}
}
